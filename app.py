...
# Estado de Hoja de trabajo (para que no se borre)
hoja_defaults = {
    "contexto_literario": "",
    "contexto_cultural": "",
    "contexto_biblico": "",
...
if modo == "Aula":
    ...
    if st.button("✅ Completar Lección 1", key="btn_completar_leccion_1"):
...
# -----------------------------
    ...
    if paso == 1:
        ...
        st.session_state.audiencia_original = st.text_area(
            ...
            height=120,
            key="alumno_audiencia"
        )
        if st.button("Siguiente → (Paso 2)", key="btn_paso1_sig"):
            if puede_avanzar(1):
                ir_a_paso(2)
            else:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("← Volver (Paso 1)", key="btn_paso2_back"):
                ir_a_paso(1)
        with c2:
            if st.button("Siguiente → (Paso 3)", key="btn_paso2_sig"):
                if puede_avanzar(2):
                    ir_a_paso(3)
                else:
        ...
        st.session_state.estructura = st.text_area(
            ...
            height=140,
            key="alumno_estructura"
        )
        c1, c2 = st.columns(2)
        with c1:
            if st.button("← Volver (Paso 2)", key="btn_paso3_back"):
                ir_a_paso(2)
        with c2:
            if st.button("Siguiente → (Paso 4)", key="btn_paso3_sig"):
                if puede_avanzar(3):
                    ir_a_paso(4)
                else:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("← Volver (Paso 3)", key="btn_paso4_back"):
                ir_a_paso(3)
        with c2:
            if st.button("✅ Terminar (y preparar Hoja de trabajo)", key="btn_paso4_end"):
                if puede_avanzar(4):
    ...
    st.divider()
    if st.button("🔄 Reiniciar respuestas del Alumno", key="btn_reset_alumno"):
